...
import static com.amazonaws.ec2.localgatewayroutetable.EventGenerator.createNotUpdatableEvent;
...
public class UpdateHandler extends BaseHandler<CallbackContext> {
    ...
    public ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        if (callbackContext == null || !callbackContext.isUpdateStarted()) {
            ...
            if (model.getOutpostArn() != null && !model.getOutpostArn().equals(existingResource.getOutpostArn())) {
                return createNotUpdatableEvent(model, "OutpostArn");
            }

            if (model.getMode() != null && !model.getMode().equals(existingResource.getMode())) {
                return createNotUpdatableEvent(model, "Mode");
            }
        ...
        try {
            if (callbackContext.getTagsToCreate() != null && !callbackContext.getTagsToCreate().isEmpty()) {
                final CreateTagsRequest createTagsRequest = CreateTagsRequest
                    ...
                    .resources(model.getLocalGatewayRouteTableId())
                    .build();
